...
const materials = new Map([
  ...
  ["4", { name: "fire", material: undefined, filename: "fire.jpg" }],
  ["5", { name: "ice", material: undefined, filename: "ice.jpg" }],
]);
...
function keyDown(event) {
  ...
  switch (key) {
    ...
    case "s":
      ...
      break;
    case "w":
      if (!player.flying) player.jumping = true;
      break;
    case " ":
...
function update() {
  ...
  if (playing) {
    ...
    var newx = playerMesh.position.x,
      newy = playerMesh.position.y;
    if (player.movingRight) {
      newx = playerMesh.position.x + 0.1;
...
async function createScene() {
  ...
  var cam = new BABYLON.FollowCamera(
    "FollowCam",
  ...
  skyboxMaterial.reflectionTexture.coordinatesMode =
    BABYLON.Texture.EQUIRECTANGULAR_MODE;
  ...
  playerModel = await BABYLON.SceneLoader.ImportMeshAsync(
    null,
    "/models/",
    "steve.glb",
    scene
  ...
  for (let m of materials.values()) {
    ...
    m.material.diffuseTexture = new BABYLON.Texture(
      "/textures/" + m.filename,
      scene
    );
  }
  ...
  for (let m of fruitModels.values()) {
    m.asset = await BABYLON.SceneLoader.LoadAssetContainerAsync(
      "/models/",
      m.filename,
      scene
    );
...
function addBlock(x, y, m, scene) {
  ...
  } else console.log("Не найден тип блока ", m);
}
...
function addObject(x, y, t, scene) {
  ...
  animShake.push({
    frame: 0,
    value: fruit.position.add(new BABYLON.Vector3(0, -0.2, 0)),
  });
  animShake.push({
    frame: 30,
    value: fruit.position.add(new BABYLON.Vector3(0, 0.2, 0)),
...
function addBullet(x, y, t, d) {
  var bullet = new BABYLON.MeshBuilder.CreateCylinder(
    "c" + x + "_" + y,
    { height: 0.7, diameterTop: 0.3, diameterBottom: 0 },
    scene
  );
  bullet.position.x = x;
  bullet.position.y = y;
...
function checkRectOverlap(rect1, rect2) {
  if (
    (rect1[0][0] < rect2[0][0] && rect2[0][0] < rect1[1][0]) ||
...
function collideExit(x, y) {
  var dist = Math.sqrt(
    Math.pow(x - endPosition.x, 2) + Math.pow(y - endPosition.y, 2)
  );
...
function loadMap(scene) {
  ...
  for (let i = 0; i < r.length; i++) {
    ...
    switch (x) {
      ...
      case "4":
      case "5":
        addBlock(col, linecount - line, x, scene);
        col++;
        break;

      case "i":
        startPosition = { x: col, y: linecount - line };
        col++;
        break;
      case "o":
        endPosition = { x: col, y: linecount - line };
        col++;
        break;
      case "c":
      case "a":
        addObject(col, linecount - line, x, scene);
        col++;
        break;
      case "m":
        addMonster(col, linecount - line, x, scene);
        col++;
        break;
    }
  }
