...
async function createEnviroment(scene: Scene) {
  ...
  const destructableCube = CreateBox('destructable-cube', { size: 2 });
  destructableCube.material = meshes[5].material;
  destructableCube.position = new Vector3(0, 3, -15);
  
  destructableCube.physicsImpostor = new PhysicsImpostor(destructableCube, PhysicsImpostor.BoxImpostor, { mass: 30 });

  let counter = 0;
  destructableCube.physicsImpostor.onCollideEvent = (collider, collidedWith) => { 
    if ((collidedWith.object as Mesh).id === 'ball') {
      counter++;
    }
    
    if (counter >= 3) {
      fracturedCube.meshes[0].position.copyFrom(destructableCube.position);
      
      fracturedCube.meshes.forEach((mesh) => {
        mesh.setParent(null);
        mesh.physicsImpostor = new PhysicsImpostor(mesh, PhysicsImpostor.BoxImpostor, { mass: 0.5 });
        mesh.material = meshes[5].material;
  });
     
      destructableCube.dispose();
    }
  };
  ...
  meshes.forEach((mesh) => {
    ...
    scene
      ...
      localCube.meshes.forEach((_mesh) => {
        ...
        _mesh.physicsImpostor = new PhysicsImpostor(_mesh, PhysicsImpostor.BoxImpostor, { mass: 0.5 });
