...
import ui from './ui/guiTexture.json'
...
function onTouchFruit(o){
  switch(o.type){
    case 'cherry':cherriesCount++; break;
    case 'apple':applesCount++; break;
    case 'peach':peachesCount++;break;
    case 'banana':bananasCount++;break;
  }

  appleText.text = applesCount;
  bananaText.text = bananasCount;
  cherryText.text = cherriesCount;
  peachText.text = peachesCount;
...
function makeUI() {
  let uiTexture = new BABYLON.GUI.AdvancedDynamicTexture.CreateFullscreenUI('UI');
...
function loadMap(scene){
  ...
  if(startPosition==undefined){
    console.log('В карте не указана стартовая позиция!')
  }
  if(endPosition==undefined){
    console.log('В карте не указана позиция конца игры!')
  }
}
