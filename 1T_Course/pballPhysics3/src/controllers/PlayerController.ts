  ...
  _calculateShoot() {
    this._scene.onPointerDown = (event) => {
      ...
      if (event.button === 0) {
        ...
        ball.physicsImpostor = new PhysicsImpostor(ball, PhysicsImpostor.SphereImpostor, { mass: 0.5 });

        ball.physicsImpostor.applyImpulse(cameraDirection.scale(20), ball.getAbsolutePosition());

        ball.physicsImpostor.onCollideEvent = (collider, collideWith) => {
          ball.dispose();
          const collidePosition = collider.physicsBody.position;
          const decal = CreateDecal("decal", collideWith.object as Mesh, {
            position: new Vector3(collidePosition.x, collidePosition.y, collidePosition.z),
            normal: raycastHit.getNormal(true),
            size: new Vector3(1, 1, 1),
          });

          decal.material = this._splatters[Math.floor(Math.random() * this._splatters.length)];
          decal.setParent(collideWith.object as Mesh);
        };

        // raycastHit.pickedMesh.physicsImpostor.applyForce(ray.direction.scale(50000), raycastHit.pickedPoint);
