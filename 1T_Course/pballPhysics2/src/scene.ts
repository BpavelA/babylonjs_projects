...
async function createEnviroment(scene: Scene) {
  ...
  meshes.forEach((mesh) => {
    if (mesh.name.includes("Walls") || mesh.name === "Floor" || mesh.name.includes("Element")) {
      mesh.setParent(null);
    ...
    if (mesh.name === "Ramp") {
      const rampBox1 = CreateBox('ramp-box-1', { width: 6.6, height: 4.15, depth: 4 });
      rampBox1.position = new Vector3(1.35, 2.17, 2.69);

      const rampBox2 = CreateBox('ramp-box-2', { width: 6.71, height: 4.14, depth: 8 });
