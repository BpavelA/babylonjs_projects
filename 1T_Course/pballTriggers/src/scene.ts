...
export async function initScene(scene: Scene) {
  ...
  const bombSounds = {
    planting: new Sound('planting-sound', './sounds/bomb_notification.mp3', scene, null, { volume: 0.2 }),
    beep: new Sound('beep-sound', './sounds/bomb_beep.mp3', scene, null, {loop: true, spatialSound: true, distanceModel: 'exponental'})
}

  const triggerForBomb = CreateBox('trigger-bomb', { width: 6.5, height: 0.25, depth: 3 });

  triggerForBomb.position = new Vector3(-1.5, 0.5, 12.5);
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    if (triggerForBomb.intersectsMesh(player.playerWrapper) && !bombWasPlanted) {
      
      bombWasPlanted = true;
      player.movementEnableStatus = false;
      console.log('Мы столкнулись с бомбой!');
      setTimeout(async () => {
        const bombObject = await SceneLoader.ImportMeshAsync('', './models/', 'c4_explosive.glb');
        const bomb = bombObject.meshes[0];
        bomb.scaling = new Vector3(0.1, 0.1, 0.1);
        bomb.position = player.playerWrapper.position.subtract(new Vector3(0, 0.9, 0));
        player.movementEnableStatus = true;
